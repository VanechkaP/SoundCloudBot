...
# Хранилище для неудавшихся треков: { user_id: { "playlist_title": ..., "playlist_artist": ..., "playlist_thumbnail": ..., "tracks": [...] } }
...
def process_thumbnail(image_path: str) -> bool:
    """Обрабатывает картинку. Возвращает True, если все ок, и False, если файл битый."""
    try:
...
async def handle_link(message: types.Message):
    ...
    try:
            ...
            if link_info.get('is_playlist'):
                ...
                for index, entry in enumerate(entries, start=1):
                    ...
                    try:
                        ...
                        # Сохраняем в список ошибок для возможности повтора
                        failed_tracks.append({"url": track_url, "title": failed_title})

                        error_report = f"⚠️ Не удалось скачать трек {index}: *{failed_title}* (Пропущен)"
                ...
                # Если есть упавшие треки, сохраняем метаданные в глобальный стейт
                if failed_tracks:
                    has_errors = True
        ...
        # Безопасно пытаемся отредактировать статус-сообщение
        edited = False
        ...
        # Если отредактировать не вышло, шлем новое, но тоже в try-except
        if not edited:
            try:
...
async def process_retry(callback: types.CallbackQuery):
    ...
    # Извлекаем кэш сохранённых ошибок
    ...
    try:
        async with download_lock:
            for index, entry in enumerate(tracks_to_retry, start=1):
                ...
                try:
                    ...
                    failed_tracks.append({"url": track_url, "title": actual_title[0]})
                    await callback.message.answer(f"⚠️ Повторная попытка не удалась: *{actual_title[0]}*")
                finally:
                    if track_data and os.path.exists(track_data['file_path']):
                        try:
                            os.remove(track_data['file_path'])
        ...
        try:
            err_msg = await callback.message.answer(
                "❌ Сбой инфраструктуры при попытке повтора. Возможно, превышены лимиты Telegram.",
                reply_markup=get_cancel_menu()
            )
            user_menus[user_id] = err_msg.message_id
        except Exception as flood_err:
            print(f"Не удалось отправить ошибку повтора из-за блокировки: {flood_err}")
    finally:
        active_downloads = max(0, active_downloads - 1)
...
# =====================================================================
# 7. ЗАПУСК БОТА (ЧИСТЫЙ LONG POLLING)
# =====================================================================

async def main():
    # На нормальном хостинге бот запускается мгновенно и работает без костылей
    print("🚀 Бот успешно запущен на новом хостинге в режиме Long Polling!")

    # Принудительно сбрасываем старые привязки/вебхуки (если Render что-то кэшировал)
    await bot.delete_webhook(drop_pending_updates=True)

    # Запускаем чистый и стабильный опрос серверов Telegram
