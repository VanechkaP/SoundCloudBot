...
# Хранилище для неудавшихся треков
            ...
            if link_info.get('is_playlist'):
                ...
                for index, entry in enumerate(entries, start=1):
                    ...
                    async def playlist_progress(percent, fetched_title=None, fetched_thumb=None):
                        if fetched_title: actual_title[0] = fetched_title
                        if fetched_thumb: actual_thumb[0] = fetched_thumb
                    ...
                    try:
                        ...
                        try:
                            await status_msg.edit_text(f"⚙️ Обработка и отправка: *{actual_title[0] or f'Трек {index}'}*...")
                        except Exception:
                            pass
                            ...
                            try:
                                ...
                                async with httpx.AsyncClient(timeout=10.0) as client:
                                    ...
                                    if response.status_code == 200:
                                        with open(thumb_path, "wb") as f: f.write(response.content)
                                        ...
                                        if playlist_thumb and thumbnail_url != playlist_thumb:
                                            ...
                                            if resp_alt.status_code == 200:
                                                with open(thumb_path, "wb") as f: f.write(resp_alt.content)
                                                process_thumbnail(thumb_path)
                            except Exception:
                                thumb_path = None
                        ...
                        await message.answer(f"⚠️ Не удалось скачать трек {index}: *{failed_title}* (Пропущен)")
                    finally:
                ...
                try: await status_msg.edit_text("⚙️ Обрабатываю аудио-файл...")
                except Exception: pass
                ...
                try: await status_msg.edit_text("📥 Отправляю аудио-файл...")
                except Exception: pass
...
async def process_retry(callback: types.CallbackQuery):
    ...
    if user_id not in failed_downloads_store:
        await callback.answer("❌ Нет аудио для повторной загрузки.", show_alert=True)
        return
    ...
    try:
        async with download_lock:
            for index, entry in enumerate(tracks_to_retry, start=1):
                ...
                try:
                    ...
                    if track_data.get('title'): actual_title[0] = track_data['title']

                    try: await status_msg.edit_text(f"⚙️ Обработка и отправка: *{actual_title[0]}*...")
                    except Exception: pass
        ...
        err_msg = await callback.message.answer("❌ Сбой инфраструктуры при попытке повтора.", reply_markup=get_cancel_menu())
        user_menus[user_id] = err_msg.message_id
    finally:
        active_downloads = max(0, active_downloads - 1)
...
# =====================================================================
# 7. ЧИСТЫЙ ЗАПУСК БОТА (LONG POLLING)
# =====================================================================
